...
public class QuanLyThucDon extends javax.swing.JFrame {
    ...
    private void displayTables() {
        ...
        for (int i = 0; i < listTable.size(); i++) {
            ...
            Object[] dt = {i + 1, tables.getName(), tables.getStatus()? "Đã có khách" : "Còn trống", tables.getNote()};
    ...
    private void displayTableDrinks() {
        ...
        for (int i = 0; i < listMenu.size(); i++) {
            ...
            Object[] dt = {i + 1, menu.getDrinkName(), menu.getPrice(), menu.getQuantity(), menu.getTotalPrice()};
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Assets/product_management_24px.png"))); // NOI18N
        ...
        tblTables.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        tblTables.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        int billId = BillsDAO.getInstance().GetUncheckInvoiceByTableId(idTable);
        Drinks drinks = (Drinks) cboDrinks.getSelectedItem();

        if (billId == -1) {
            BillsDAO.getInstance().Insert(idTable, AccountDAO.getInstance().GetAccount().getId(), true);

            OrdersDAO.getInstance().Insert(drinks.getId(), BillsDAO.getInstance().GetMaxIdInvoice(), Integer.parseInt(spnAmount.getValue().toString()));
        } else {
            OrdersDAO.getInstance().Insert(drinks.getId(), billId, Integer.parseInt(spnAmount.getValue().toString()));
        }
        Connection con = DbUtility.openConnection();
        PreparedStatement pstmt;
        try {
            pstmt = con.prepareStatement("update tblTables set status=1 where id=?");
    ...
    private void tblTablesMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblTablesMouseClicked
        ...
        if ((tblTables.getValueAt(row, 2) + "").equals("Đã có khách")) {
            btnPay.setEnabled(true);
        } else {
            btnPay.setEnabled(false);
        }
        List<Tables> listTable = TablesDAO.getInstance().LoadListTables();
        idTable = listTable.get(row).getId();
        displayTableDrinks();
    ...
    private void btnPayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPayActionPerformed
        try {
            // TODO add your handling code here:
            int billId = BillsDAO.getInstance().GetUncheckInvoiceByTableId(idTable);
            BillsDAO.getInstance().Update(billId, totalPrice);
            Connection con = DbUtility.openConnection();
            PreparedStatement pstmt = con.prepareStatement("update tblTables set status=0 where id=?");
            pstmt.setInt(1, idTable);
            pstmt.executeUpdate();
            displayTables();
            displayTableDrinks();
